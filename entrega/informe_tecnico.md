...
**Deduplicación por contenido.** El índice contiene 91.021 fragmentos pero 87.424 textos únicos: 3.597 (4,0 %) repiten contenido, con un grupo mayor de 11 copias, en dos patrones -filas tabulares idénticas repartidas entre documentos y secciones repetidas dentro de un mismo informe-. Puesto que §10.2.1 evalúa la relevancia sobre el campo `text`, dos fragmentos idénticos ocuparían dos posiciones con la misma información. La clave de comparación normaliza únicamente el espaciado; no se normalizan mayúsculas ni puntuación, porque dos textos que difieren en ellas difieren realmente.
...
**Reconocimiento de entidades.** Se emplea `urchade/gliner_multi-v2.1`, revisión `443d26d6`, licencia Apache 2.0. Es un encoder bidireccional construido sobre mDeBERTa-v3, de modo que no incurre en la restricción de §4.2, y su naturaleza *zero-shot* permite obtener tipos que un reconocedor clásico no produciría, como tecnologías o normas. Se definieron nueve tipos de entidad con umbral de confianza 0,5. La unidad de entrada son subventanas de los fragmentos del índice con presupuesto de 320 tokens: los fragmentos tienen mediana de 692 tokens y la ventana del modelo es de 384, de modo que pasarle fragmentos completos habría dejado sin analizar la segunda mitad de la mayoría sin ningún aviso. Se procesaron 127.070 subventanas de 48.087 fragmentos en 1 h 57 min sobre GPU T4, sin ninguna truncada.

**Extracción de relaciones.** Se realiza mediante patrones lingüísticos sobre el texto que media entre dos entidades del mismo fragmento, vía que §7.2 autoriza expresamente junto a los modelos de clasificación y las dependencias sintácticas. No se emplea ningún modelo generativo.

**Canonicalización.** Cada entidad se reduce a una clave determinista y se aplican alias curados manualmente, de modo que las variantes de una misma organización o país converjan en un único nodo.

**Trazabilidad.** Cada arista conserva el `doc_id` y el `chunk_id` de origen, conforme a §7.2, lo que permite rastrear la evidencia textual de cada relación. Los 3.209 identificadores de fragmento que aparecen en el grafo existen en el `metadata.jsonl`. Aportan evidencia 484 de los 1.818 documentos (26,6 %), repartidos de forma equilibrada entre los tres fenómenos: 151 de F1, 181 de F2 y 152 de F3.

El archivo se somete a siete pruebas automáticas que verifican estructura, trazabilidad, canonicalización, integridad al releerse, cobertura, ausencia de dependencias generativas e interoperabilidad. Esta última se añadió tras detectar que un archivo que superaba las seis anteriores no abría en ningún visor: las aristas compartían trece valores de identificador, y GraphML los exige globalmente únicos. Ninguna prueba lo detectaba porque la biblioteca de grafos interpreta ese identificador como clave por par de nodos, de modo que toda comprobación que partiera del grafo ya cargado era ciega por construcción. La prueba añadida lee el XML en crudo.

El grafo no se integró en el flujo de recuperación. §8.5 lo plantea como posibilidad y no como requisito; integrarlo modificaría el archivo de resultados y obligaría a repetir la verificación de reproducibilidad, y su efecto sobre las métricas no es medible sin el conjunto de referencia.
...
La reproducibilidad se comprobó eliminando el archivo de resultados y regenerándolo desde cero en dos sesiones independientes, con descarga fresca del modelo y versiones distintas de biblioteca, obteniendo un archivo idéntico byte a byte: 801.865 bytes, 50 líneas, SHA-256 `1ec6f8200bd7...`. El entorno de referencia queda documentado en el manifiesto.
...
La construcción del índice desde el corpus -no la reproducción de los resultados- requiere además **Tesseract OCR 5.4.0** con los idiomas español e inglés, dependencia de sistema que los gestores de paquetes de Python no capturan. La ejecución que generó el archivo entregado se realizó en un entorno sin Tesseract y sin bibliotecas de extracción de PDF instaladas, lo que confirma que no interviene en la reproducción.
